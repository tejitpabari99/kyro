...
14. **Does a workout exercise's `notes` participate in the update-routine material-change check (02 §14.4), given 04 §2.4's write-back explicitly copies it back?** These are two different questions — 02 §14.4's own enumerated material-change list never mentions `notes`, so a notes-only edit never triggers the prompt; but once the prompt fires for some *other* material reason and the user accepts "Update routine," `notes` is still overwritten from the workout's final structure, exactly like every other structural field 04 §2.4 lists. Implemented in M3-06 (`domain/routine-diff.ts`'s `RoutineDiffWorkoutExerciseLike`/`RoutineDiffRoutineExerciseLike` carry no `notes` field at all, so a diff input can't represent a notes-only change; `RoutineRepositoryImpl.updateFromWorkout` copies `workout_exercises.notes` unconditionally on write-back).
